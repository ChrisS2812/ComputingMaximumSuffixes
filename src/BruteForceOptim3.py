...
def check_alg_for_root_comp(comp_index, words, c: bitarray):
    ...
    if (check_alg(root_node.children[0], smaller_list, c_smaller, first_rel_char_smaller)
            and check_alg(root_node.children[1], equal_list, c_equal, 0)
            and check_alg(root_node.children[2], bigger_list, c_bigger, 0)):
        return root_node
    else:
        return
...
def check_alg(current_node, words, c: bitarray, first_rel_char):
    global WD_TIME, COPY_TIME, NR_CALLS
    NR_CALLS += 1
    ...
    if current_node.depth < m-1:
        ...
        for i in [i for i, bit in enumerate(c) if bit]:
            ...
            if (check_alg(current_node.children[0], smaller_list, c_smaller, first_rel_char_smaller) and
                    check_alg(current_node.children[1], equal_list, c_equal, first_rel_char) and
                    check_alg(current_node.children[2], bigger_list, c_bigger, first_rel_char)):
                return True
        return False
