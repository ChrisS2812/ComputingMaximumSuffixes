...
def check_alg_for_root_comp(root_comp, words, comps):
    ...
    if (check_alg(root_node.children[0], smaller_list, comps)
            and check_alg(root_node.children[1], equal_list, comps)
            and check_alg(root_node.children[2], bigger_list, comps)):
...
def check_alg(current_node, words, comps):
    ...
    if not current_node.is_leaf:
        ...
        for c_new in comps:
            ...
            if (check_alg(current_node.children[0], smaller_list, comps) and
                    check_alg(current_node.children[1], equal_list, comps) and
                    check_alg(current_node.children[2], bigger_list, comps)):
                return True
        return False
